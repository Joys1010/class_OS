...
palloc_get_multiple (enum palloc_flags flags, size_t page_cnt)
{
	...
	if (pallocator == 0 ) { //first fit (default)
	...
	else if ( pallocator == 1 ) { //next fit 1
		
		my_pallocator = 1;
		page_idx = bitmap_scan_and_flip_nextfit (pool->used_map, last_idx, page_cnt, false);

	}

	else if ( pallocator == 2 ) { //best fit 2 
		
		my_pallocator = 2;
		page_idx = bitmap_scan_and_flip_bestfit (pool->used_map, 0, page_cnt, false);
	}

	else if ( pallocator == 3 ) { //buddy system 3
...
palloc_free_multiple (void *pages, size_t page_cnt) 
{
	...
	if (pallocator == 3) { //only execute when Buddy System
		merge_hole(page_idx,page_cnt);
	}
